...
export const TradeTape: React.FC<TradeTapeProps> = ({ trades }) => {
  return (
    <div className="flex flex-col h-full bg-slate-950/60 rounded-xl border border-slate-800/80 overflow-hidden shadow-inner">
      ...
      {/* Column Headers */}
      <div className="grid grid-cols-4 px-4 py-1.5 border-b border-slate-800/50 text-[11px] font-mono text-slate-500 bg-slate-950/40 select-none">
        <div>TIME</div>
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-900/60">
        {trades.length === 0 ? (
          ...
          trades.map((trade) => {
            ...
            const timeStr = date.toTimeString().split(' ')[0] + '.' + String(Math.floor((trade.ts_ns / 1_000_000) % 1000)).padStart(3, '0');

            return (
              <div
                key={`${trade.trade_id}-${trade.seq}`}
                className={`grid grid-cols-4 px-4 py-1.5 text-xs font-mono items-center transition-colors hover:bg-slate-900/50 ${
